...
def test_multi_product_scope_authorizes_only_exact_product_and_stage(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    root = tmp_path / "repo"
    root.mkdir()
    ...
    path.write_text(json.dumps({
        ...
        "head_sha": "b" * 40,
        "authorized_scopes": [{
            "product_number": "23-9403-490",
            "allowed_stage_keys": ["CREATE_CORE", "BROADCAST_5_12"],
        }],
        "concurrent_shijiu_writer_observed": False,
        "exclusive_window_confirmed": True,
        "confirmation_basis": "shared_scheduler",
        "issued_at": (current - timedelta(minutes=1)).isoformat(),
        "expires_at": (current + timedelta(minutes=10)).isoformat(),
    }), encoding="utf-8")
    monkeypatch.setattr(
        "mikihouse_luyao.shijiu_writer_mutex.current_head_sha", lambda value: "b" * 40
    )
    assert validate_writer_mutex_evidence(
        path,
        root=root,
        product_number="23-9403-490",
        stage_key="CREATE_CORE",
    )["status"] == "VERIFIED_BEFORE_WRITE"
    with pytest.raises(LiveImportError, match="product/stage"):
        validate_writer_mutex_evidence(
            path,
            root=root,
            product_number="23-9403-490",
            stage_key="DETAIL_PICS_1_8",
        )

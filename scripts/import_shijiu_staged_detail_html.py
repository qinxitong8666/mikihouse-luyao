...
from mikihouse_luyao.shijiu_staged_detail_html import (
    DETAIL_HTML_MODE,
    DETAIL_HTML_WRITE_CONFIRMATION,
    DetailHtmlStagedRunner,
    analyze_frozen_detail_readback,
    build_detail_html_conclusion,
    load_detail_html_candidate,
    make_detail_html_clients,
    select_detail_html_candidate,
)
from mikihouse_luyao.shijiu_staged_media_complete import build_next_20_frozen_plan
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        description="Validate Shijiu staged detail pictures and final HTML on one new product"
    )
...
def verify_live_source(item: dict) -> dict:
    ...
    if set(actual) != set(expected):
        raise RuntimeError("live source SKU set drift for detail/HTML candidate")
...
def main(argv: list[str] | None = None) -> int:
    ...
    selection_path = ROOT / "config/shijiu_staged_detail_html_single.json"
    checkpoint_path = ROOT / "state/shijiu_staged_detail_html_single_checkpoint.json"
    base = ROOT / "deliverables/shijiu_import"
    report_path = base / "staged_detail_html_validation_report.json"
    readbacks_path = base / "staged_detail_html_readbacks.json"
    candidate_path = base / "staged_detail_html_candidate.json"
    preflight_path = base / "staged_detail_html_resource_preflight.json"
    conclusion_path = base / "staged_detail_html_capacity_conclusion.json"
    forensics_path = base / "staged_detail_html_false_negative_forensics.json"
    readiness_path = base / "staged_detail_html_readiness.json"
    next_20_path = base / "staged_detail_html_next_20_frozen_plan.json"
    ...
    if selection_path.exists():
        ...
        item = load_detail_html_candidate(master, special, category, selection)
    else:
        item, selection = select_detail_html_candidate(ROOT, master, special, mapping, category)
    ...
    client, ui, browser_evidence = make_detail_html_clients(
        args.browser_private_dir, contract_path
    )
    runner = DetailHtmlStagedRunner(
        client,
        ui,
        item,
        special,
    ...
    def write_outcomes() -> dict:
        ...
        forensics = analyze_frozen_detail_readback(item, checkpoint)
        write_json_atomic(forensics_path, forensics)
        conclusion = build_detail_html_conclusion(checkpoint, forensics)
        ...
        readiness = {
            ...
            "status": (
                ...
                if ready else "NOT_READY_SINGLE_PRODUCT_FLOW_INCOMPLETE"
            ),
    ...
    if args.preflight_resources:
        ...
        return 0
    if args.confirm != DETAIL_HTML_WRITE_CONFIRMATION:
